...
library LETSBaseInitializeLib {
    function initialize(address sc, bytes calldata params) internal returns(bytes4[] memory selectors) {
        (
            string memory name,
        ...
        LETSBaseSchema.LETSBaseLayout storage $base = LETSBaseStorage.LETSBaseSlot();
