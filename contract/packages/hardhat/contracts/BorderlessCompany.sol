...
import {IBorderlessCompany} from "./interfaces/IBorderlessCompany.sol";
import {EventBorderlessCompany} from "./interfaces/EventBorderlessCompany.sol";
import {ErrorBorderlessCompany} from "./interfaces/ErrorBorderlessCompany.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract BorderlessCompany is
    IBorderlessCompany,
    EventBorderlessCompany,
    ErrorBorderlessCompany,
    Initializable
{
    // ============================================== //
    //                  Storage                      //
    ...
    function initialService(
        address[] calldata services_
    ) external override onlyRegister returns (bool completed_) {
        if (
            _getService(1) != address(0) ||
            _getService(2) != address(0) ||
            _getService(3) != address(0)
        ) revert AlreadyInitialService(msg.sender);
    ...
    function getService(
        uint256 index_
    ) external view override onlyAdmin returns (address service_) {
        require(
            index_ >= 0 && _getService(index_) != address(0),
            "Error: BorderlessComapny/Invalid-Index"
        );
    ...
    function releaseRole(
        address account_,
        bool isAdmin_
    ) external override onlyAdmin returns (bool released_) {
        if (account_ == address(0)) revert InvalidAddress(account_);
    ...
    function _getService(
        uint256 index_
    ) internal view returns (address service_) {
        service_ = _services[index_];
    }

    function _assignmentRoleAdmin(
        address account_
    ) internal returns (bool assigned_) {
    ...
    function _assignmentRoleMember(
        address account_
    ) internal returns (bool assigned_) {
    ...
    function _releaseRoleAdmin(
        address account_
    ) internal returns (bool released_) {
    ...
    modifier onlyRegister() {
        require(
            ...
            "Error: BorderlessCompany/Only-Register"
        );
        _;
    }
}
