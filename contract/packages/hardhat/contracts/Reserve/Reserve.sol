...
import {EventReserve} from "./interfaces/EventReserve.sol";
import {ErrorReserve} from "./interfaces/ErrorReserve.sol";
    ...
    UUPSUpgradeable
{
    ...
    function reservation(
        address account_
    ) external override onlyAdmin returns (bool listed_) {
    ...
    function isWhitelisted(
        address account_
    ) external view override returns (bool listed_) {
        if (account_ == address(0)) revert InvalidAddress(account_);
    ...
    function reserverOf(
        uint256 index_
    ) external view onlyAdmin returns (address reserver_) {
        if (index_ <= 0 || index_ > _lastIndex) revert InvalidIndex(index_);

        reserver_ = _reserverOf(index_);
    }

    function reserversOf()
        external
        view
        onlyAdmin
        returns (address[] memory reservers_)
    {
        reservers_ = new address[](_lastIndex);

        for (uint256 i = 1; i <= _lastIndex; i++) {
            if (_reserverOf(i) != address(0))
                reservers_[i - 1] = _reserverOf(i);
        }
    }
    ...
    function _cancelWhitelist(
        address account_
    ) internal returns (bool listed_) {
        uint256 _index = 1;

        for (_index; _index <= _lastIndex; _index++) {
            if (_reserverOf(_index) == account_)
                _reservers[_index] = address(0);
            if (_isWhitelisted(account_)) _whitelist[account_] = false;

            if (
                _reserverOf(_index) == address(0) && !_isWhitelisted(account_)
            ) {
    ...
    function _isWhitelisted(
        address account_
    ) internal view returns (bool listed_) {
        listed_ = _whitelist[account_];
    }
    ...
    function _reserverOf(
        uint256 index_
    ) internal view returns (address reserver_) {
