...
2. `ロール付与`を管理できる機能 // TODO: ロール管理（Access control）の課題検討
3. `Borderless.company`の管理・運営をする機能（Operation・Manager）
...
interface IBorderlessCompany { // Note: IBorderlessCompny is an feature interface
    ...
    */
    function assignmentRole(address account_, isAdmin_) external returns(bool assigned_);
    ...
    */
    function releaseRole(address account_, isAdmin_) external returns(bool released_);
}
```
...
1. `Error: BorderlessCompany/Invalid-Param` 不正なパラメータのリバート
2. `Error: BorderlessCompany/Already-Initial-Service` 初期設定済み機能の不正設定リバート
3. `Error: BorderlessCompany/Already-Assignment-Role` 既にロール付与していることのリバート
4. `Error: BorderlessCompany/Already-Release-Role` 既にロール付与者ではないことのリバート
5. `Error: BorderlessCompany/Only-Admin` `Admin`以外の不正な呼び出し者のリバート
6. `Error: BorderlessCompany/Only-Register` `Register`以外の不正な呼び出し者のリバート（初期セットアップで使用する）
