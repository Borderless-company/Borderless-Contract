...
import {EventNonFungibleTokenTYPE721} from "src/interfaces/Services/LETSService/TokenStandards/NonFungibleTokenTYPE721/EventNonFungibleTokenTYPE721.sol";
import {ErrorNonFungibleTokenTYPE721} from "src/interfaces/Services/LETSService/TokenStandards/NonFungibleTokenTYPE721/ErrorNonFungibleTokenTYPE721.sol";
...
contract NonFungibleTokenTYPE721 is
    ERC721,
    EventNonFungibleTokenTYPE721,
    ErrorNonFungibleTokenTYPE721
{
    ...
    function _incrementLastID(
        uint256 currentTokenID_
    ) internal returns (bool updated_, uint256 tokenID_) {
        uint256 _currentID = currentTokenID_;
        _lastTokenID++;
        uint256 _updateID = _currentTokenID();

        if (_currentID + 1 == _updateID)
            (updated_, tokenID_) = (true, _updateID);
    }

    // -- 標準機能 -- //

    function mint() external validateCall {
        if (_holoderOf(msg.sender))
            revert AlreadyMinted(msg.sender, name(), symbol());
    ...
    function tokenURI(
        ...
        require(
            ...
            "Error: NonFungibleToken721/Only-TokenHolder"
        );
        return
            string(abi.encodePacked(ERC721.tokenURI(tokenId_), _baseExtension));
    }
    ...
    function setupPublicSale(
        ...
        if (saleEnd_ < block.timestamp)
            revert InvalidTime(saleStart_, saleEnd_);
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == _admin,
            "Error: NonFungibleTokenTYPE721/Only-Admin"
        );
        _;
    }
    ...
    modifier validateCall() {
        require(
            _validateCall(),
            "Error: NonFungibleTokenTYPE721/Invalid-Caller"
        );
        _;
    }
    ...
    function _checkSale() internal view returns (bool active_) {
        if (
            _saleActive ||
            (_saleStart <= block.timestamp && _saleEnd >= block.timestamp)
        ) active_ = true;
    }
}
