...
import {BorderlessCompany, ISCT} from "src/BorderlessCompany.sol";
    ...
    ErrorRegisterBorderlessCompany
{
    ...
    function _createBorderlessCompany(
        ...
        if (!_updated) revert DoNotCreateSmartCompany(msg.sender);
        ...
        if (_completed)
            emit NewSmartCompany(info_.founder, info_.companyAddress, _index);
    ...
    function _setupService(
        ...
        completed_ = ISCT(company_).initialService(_services);
    }
